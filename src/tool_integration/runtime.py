...
from tool_integration.events import EventBus, ToolIntegrationEvent, ToolIntegrationEventType
from tool_integration.jobs import JobRecord, JobRuntime
from tool_integration.schema import SchemaError, validate_schema
from tool_integration.tools import SwitchPolicy, Tool, ToolContext, ToolRegistry, new_call_id
...
class ToolRuntime:
    ...
    def _invoke_async(
        ...
        args: dict[str, Any],
        call_id: str,
        view_mode: str = "background",
    ) -> dict[str, Any]:
        job = self.jobs.create(tool.spec.name, call_id, args, view_mode=view_mode)  # type: ignore[arg-type]
        self.event_bus.publish(
            ToolIntegrationEvent(
                event_type=ToolIntegrationEventType.TOOL_JOB_STARTED,
                source=tool.spec.name,
                call_id=call_id,
                job_id=job.job_id,
                payload={"tool": tool.spec.name, "args": args, "view_mode": view_mode},
            )
        )
    ...
    def _invoke_switchable(self, tool: Tool, args: dict[str, Any], call_id: str) -> dict[str, Any]:
        ...
        self.event_bus.publish(
            ToolIntegrationEvent(
                event_type=ToolIntegrationEventType.TOOL_JOB_FOREGROUND_WAIT_STARTED,
                source=tool.spec.name,
                call_id=call_id,
                job_id=job.job_id,
                payload={"timeout_seconds": policy.foreground_timeout_seconds},
            )
        )
            ...
            if job.state in ("succeeded", "failed", "cancelled"):
                if job.state == "succeeded":
                    return {
                        ...
                        "result": job.result,
                        "job_id": job.job_id,
                        "completed_as_foreground": True,
                    }
                return self._error(
                    tool.spec.name,
                    call_id,
                    job.state,
                    job.error or job.state,
                    job.traceback,
        ...
        self.event_bus.publish(
            ToolIntegrationEvent(
                event_type=ToolIntegrationEventType.TOOL_JOB_SWITCHED_TO_BACKGROUND,
                source=tool.spec.name,
                call_id=call_id,
                job_id=job.job_id,
                payload={"reason": "foreground_timeout"},
            )
        )
    ...
    def _run_job(self, tool: Tool, job: JobRecord):
        ...
        try:
            ...
            evt = (
                ToolIntegrationEventType.TOOL_JOB_CANCELLED
                if job.cancel_requested
                else ToolIntegrationEventType.TOOL_JOB_FAILED
            )
            self.event_bus.publish(
                ToolIntegrationEvent(
                    event_type=evt,
    ...
    def _error(
        ...
        self.event_bus.publish(
            ToolIntegrationEvent(
                event_type=ToolIntegrationEventType.TOOL_CALL_FAILED,
                source=tool_name,
                call_id=call_id,
                payload=out,
            )

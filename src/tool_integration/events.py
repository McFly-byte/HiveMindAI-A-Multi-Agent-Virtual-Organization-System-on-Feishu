"""Tool-layer events, aligned structurally with ``agent_runtime.events``.

``agent_runtime`` models *why an agent run started* (:class:`AgentTriggerEvent`).
...
class ToolIntegrationEvent(BaseModel):
    ...
    Mirrors the shape of :class:`~agent_runtime.events.AgentTriggerEvent`:
...
class EventBus:
    """In-process pub/sub (same responsibility as legacy ``tooltest`` bus)."""

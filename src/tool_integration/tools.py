...
from tool_integration.events import ToolIntegrationEvent, ToolIntegrationEventType, EventBus
...
class ToolRegistry:
    ...
    def to_llm_tools(self, allowed_tools: list[str] | None = None, include_meta: bool = True) -> list[dict[str, Any]]:
        ...
        for tool in self.tools.values():
            if allowed_tools and tool.spec.name not in allowed_tools:
                continue
...
class ToolContext:
    ...
    def checkpoint(self, payload: dict[str, Any]):
        ...
        self.emit(ToolIntegrationEventType.TOOL_JOB_CHECKPOINT.value, payload)
